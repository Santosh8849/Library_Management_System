...
<body>
    ...
    <input type="text" id="cat_name" name="cat_name" value="<?php echo $cat_name; ?>" class="form-control" required>
    <div id="cat_name_error" style="color: red;"></div>
</div>

<script>
    document.getElementById('cat_name').addEventListener('input', function () {
        var catName = this.value;
        var catNameError = document.getElementById('cat_name_error');
        if (/^[a-zA-Z0-9\s\-_!@#$%^&*()+=<>?/\|'"{}\[\]~`]{1,30}$/.test(catName)) {
            catNameError.textContent = '';
        } else {
            catNameError.textContent = 'Category name should be alphanumeric with some special characters allowed and have a maximum length of 30 characters.';
        }
    });
</script>
...
if (isset($_POST['update'])) {
    ...
    <script type="text/javascript">
        ...
        window.location.href = "../category/manage_cat.php";
    </script>
<?php
}
?>